...
import { EXAM_CONFIG } from "@/lib/ap-statistics/config";

describe("AP Statistics Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM172");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("ap-statistics");
  });

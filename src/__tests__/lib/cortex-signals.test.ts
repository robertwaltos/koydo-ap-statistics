...
import { emitPrismSignal } from "@/lib/ap-statistics/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "ap-statistics" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "ap-statistics" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "ap-statistics" });
